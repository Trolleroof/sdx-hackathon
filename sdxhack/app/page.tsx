  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (data.error) {
        ...
        // Format Parallel API response
        const result = data.results?.[0]?.content || JSON.stringify(data, null, 2);
        setMessages((prev) => [
          ...prev,
      ...
      <main className="flex w-full">
        ...
        <aside className="flex w-1/4 min-h-screen flex-col bg-white">
          <div className="border-b border-zinc-200 px-4 py-3">
            <h2 className="text-sm font-semibold text-zinc-900">Hackathon Chat</h2>
          </div>
          
          <div className="flex-1 space-y-3 overflow-y-auto p-4">
            {messages.length === 0 ? (
              <div className="text-xs text-zinc-500">Ask me about stock ideas...</div>
            ) : (
          ...
          <form onSubmit={handleSubmit} className="border-t border-zinc-200 p-3">
            <div className="flex gap-2">
              <input
                ...
                placeholder="Search for stock ideas..."
                disabled={isLoading}
